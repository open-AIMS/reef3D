function sc_wrapper_demogr(f, pdens, qsizes, outpath, campose)
    results=[];
    ...
    if exist(fullfile(campose,f.name), 'file')==2
        ...
        for x=(1:length(qsizes))
            ...
            site=repmat(char(reeftran(3)),n,1);
            date=repmat(char(reeftran(2)),n,1);
            repeat=repmat(char(reeftran(4)),n,1);
            x=inPoints(:,1);
            y=inPoints(:,2);
            r=table(reefname,date,site,repeat,qsize,x,y,rgsty, slope, aspect, rangez, sdevz, rgstyXY, concavity, meandevz); 
            %% PLOT RESULTS
